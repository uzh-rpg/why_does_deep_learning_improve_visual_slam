...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mMinFrames = 0;
    mMaxFrames = settings->fps();  // can change from settings->fps() to something lower
    mbRGB = settings->rgb();
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, const int64_t &timestampNs, string filename)
{
    mImGray = im;
    if(mImGray.channels()==3)
    {
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray,timestamp,timestampNs,mpIniORBextractor,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
        else
            mCurrentFrame = Frame(mImGray,timestamp,timestampNs,mpORBextractorLeft,mpORBVocabulary,mpCamera,mDistCoef,mbf,mThDepth);
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            if(mState==OK)
            {
                ...
                else
                {
                    ...
                    if(!bOK)
                    {
                        cout << "Track with motion model failed, trying reference keyframe!!\n";
                        bOK = TrackReferenceKeyFrame();

                        if(!bOK)
                        {
                            cout << "Track with reference keyframe failed!!\n";
                        }
                    }
                }
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            // std::cout << "Frame " << mCurrentFrame.mnId << " need new KF: " << bNeedKF << std::endl;
            ...
            if(bNeedKF && (bOK || (mInsertKFsLost && mState==RECENTLY_LOST &&
                                   (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD))))
            {
                std::string msg = "Trying to set frame at time " + to_string(mCurrentFrame.mTimeStamp) + " as new KeyFrame";
                ...
                msg = "Frame at time " + to_string(mCurrentFrame.mTimeStamp) + " is new KeyFrame";
...
{
    // debug printing
    int nInMap = mpReferenceKF->GetNumberMPs();
    std::string msgmp = "[TrackReferenceKeyFrame] Current time: " + to_string(mCurrentFrame.mTimeStamp) + ". Reference KF at time: " + to_string(mpReferenceKF->mTimeStamp)
                + " contains " + to_string(nInMap) + " MapPoints";
    std::cout << msgmp << std::endl;

    // const vector<MapPoint*> dbMapPoints = mpReferenceKF->GetMapPointMatches();
    // int mcnt = 0;
    // for(int i = 0; i < mpReferenceKF->N; i++)
    // {
    //     MapPoint* pMP = dbMapPoints[i];
    //     if(pMP && !pMP->isBad())
    //     {
    //         mcnt++;
    //         std::cout << mcnt << "] (" << mpReferenceKF->mvKeysUn[i].pt.x << ", " 
    //         << mpReferenceKF->mvKeysUn[i].pt.y << ") " << std::endl;
    //     }
    // }
    // end

    // We perform first an Opticalflow / ORB matching with the reference keyframe
    // If enough matches are found we setup a PnP solver
    ORBmatcher matcher(0.7,true);

    vector<MapPoint*> vpMapPointMatches;

    int64_t dt = mCurrentFrame.mTimeStampNs - mpReferenceKF->mTimeStampNs;

    int nmatches = 0;

    // ORB matching = 0, optical flow matching = 1, ORB matching with flow prior = 2, optical flow matching with weights = 3.
    int method = 3;
    ...
    else if (method == 1)
    {
        if (dt <= dt_flow_thr)
        {
            nmatches = matcher.SearchByOpticalFlow(mCurrentFrame, mpReferenceKF, vpMapPointMatches);

            if (nmatches < 10) // True also when optical flow is not loaded
            {
                // cout << "[TrackReferenceKeyFrame]: Search by Optical Flow failed. Using BoW\n";
    ...
    else if (method == 2)
    {
        ...
        {
            int th = 20;
            int c = static_cast<int>(dt) / 50000000;

            nmatches = matcher.SearchWithFlowPrior(mCurrentFrame, mpReferenceKF, vpMapPointMatches, c*th);

            if (nmatches < 0) // True when optical flow is not loaded
            {
                // cout << "[TrackReferenceKeyFrame]: Search by SearchWithFlowPrior failed. Using BoW\n";
    ...
    else if (method == 3)
    {
        ...
        if (dt <= dt_flow_thr)
        {
            nmatches = matcher.SearchByOpticalFlow(mCurrentFrame, mpReferenceKF, vpMapPointMatches, withWeights);
    ...
    //         dbNMatches++;
    // }
    // std::cout << "[TrackReferenceKeyFrame] vpMapPointMatches = " << dbNMatches << ", nmatches = "<< nmatches << std::endl;
    // end

    // if(nmatches<10)
    if(nmatches<7)
    {
        cout << "TRACK_REF_KF: Less than 7 matches!!\n";
        return false;
    }
    ...
    Optimizer::PoseOptimization(&mCurrentFrame, withWeights);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    {
        nmatches = matcher.SearchByOpticalFlow(mCurrentFrame,mLastFrame);

        std::string msg = "[Matches with optical flow]: " + std::to_string(nmatches);
        Verbose::PrintMess(msg, Verbose::VERBOSITY_DEBUG);

    }
    else if (method == 2)
    {
        nmatches = matcher.SearchWithFlowPrior(mCurrentFrame,mLastFrame,th);
        if(nmatches<15) // If few matches, uses a wider window search
        {
            Verbose::PrintMess("[SearchWithFlowPrior] Not enough matches, wider window search!!", Verbose::VERBOSITY_NORMAL);
            fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));

            nmatches = matcher.SearchWithFlowPrior(mCurrentFrame,mLastFrame,2*th);
            Verbose::PrintMess("[SearchWithFlowPrior] Matches with wider search: " + to_string(nmatches), Verbose::VERBOSITY_NORMAL);
        }
    }
    else if (method == 3)
    {
        ...
        nmatches = matcher.SearchByOpticalFlow(mCurrentFrame,mLastFrame,withWeights);
    ...
    // std::string msgMaO = "[TrackWithMotionModel] matches after optimization: " + std::to_string(nmatchesMap);
    // debug
    // std::cout << msgMaO << std::endl;
    // end
    // Verbose::PrintMess(msgMaO, Verbose::VERBOSITY_DEBUG);
...
bool Tracking::TrackLocalMap()
{
    ...
    // debug
    // std::string msg0 = "[TrackLocalMap] Tracked points before PO: " + std::to_string(aux1)
    //             + ", outliers before PO: " + std::to_string(aux2);
    ...
    // debug
    // std::string msg1 = "[TrackLocalMap] Tracked points after PO: " + std::to_string(aux1)
    //             + ", outliers after PO: " + std::to_string(aux2);
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(mSensor==System::MONOCULAR)
        thRefRatio = 0.90f;  // can change from 0.90f to 0.95f
    ...
    if(((c1a||c1b||c1c) && c2)||c3 ||c4)
    {
        ...
        else
        {
            ...
            {
                Verbose::PrintMess("[NeedNewKeyFrame] localmap is busy", Verbose::VERBOSITY_DEBUG);
                return false;
            }
        }
    }
