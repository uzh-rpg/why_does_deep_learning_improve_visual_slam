...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const int64_t &timestampNs, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,timestampNs,filename);
...
void System::SaveAllMapTrajectoriesEuRoC(const string &filename)
{
    cout << endl << "Saving all map trajectories" << endl;
    ...
    for(Map* pMap :vpMaps)
    {
        if(pMap)
        {
            ...
            std::string mapFilename = filename + "_map_" + std::to_string(idx) + ".txt";
            SaveTrajectoryEuRoC(mapFilename, pMap);
        }
    }
}
