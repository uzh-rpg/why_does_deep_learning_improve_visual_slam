...
void LocalMapping::CreateNewMapPoints()
{
    ...
    // debug
    // std::string msgdb = "[CreateNewMapPoints] Current KF: " + to_string(mpCurrentKeyFrame->mTimeStamp)
    //                 + ", Neigh KFs: ";
    // for(size_t i=0; i<vpNeighKFs.size(); i++)
    //     msgdb += to_string(vpNeighKFs[i]->mTimeStamp) + " ";
    // std::cout << msgdb << std::endl;
    // end
    ...
    // for debug printing
    int ratioBaselineDepthSuccess = 0;
    ...
    for(size_t i=0; i<vpNeighKFs.size(); i++)
    {
        ...
        else
        {
            ...
            {
                std::string msg = "ratioBaselineDepth = " + to_string(ratioBaselineDepth) + " < 0.01";
                Verbose::PrintMess(msg, Verbose::VERBOSITY_DEBUG);

                continue;
            }

            ratioBaselineDepthSuccess++;
        ...
        // for printing
        int withEnoughParallax = 0;
        int triangSucceded = 0;
        int inFront = 0;
        int reprErrOk = 0;
        int closeEnough = 0;
        int scaleConsistent = 0;
        int success = 0;
        ...
        // std::string msg = "[CreateNewMapPoints] success / scaleConsistent / closeEnough / reprErrOk / inFront / triangSucceded / withEnoughParallax / matches : " + 
        //     to_string(success) + " / " + to_string(scaleConsistent) + " / " + to_string(closeEnough) + " / " + to_string(reprErrOk) + " / " +
        //     to_string(inFront) + " / " + to_string(triangSucceded) + " / " + to_string(withEnoughParallax) + " / " + to_string(nmatches);
    ...
    // debug
    // int nInMap = mpCurrentKeyFrame->GetNumberMPs();
    // std::string msgmp = "[CreateNewMapPoints] Current KF at time: " + to_string(mpCurrentKeyFrame->mTimeStamp)
    //             + " contains " + to_string(nInMap) + " MapPoints";
    // std::cout << msgmp << std::endl;
    // end
    
    // std::string msgrb = "[CreateNewMapPoints] Keyframes Processed / Available : " + to_string(ratioBaselineDepthSuccess) + " / " + to_string(vpNeighKFs.size());
    // debug
    // std::cout << msgrb << std::endl;
    // Verbose::PrintMess(msg, Verbose::VERBOSITY_DEBUG);
    // end
}
...
void LocalMapping::KeyFrameCulling()
{
    ...
    std::string msg = "[KeyFrameCulling] Removed / Num. Local KFs = " + to_string(rmKF) + " / " + to_string(vpLocalKeyFrames.size());
    Verbose::PrintMess(msg, Verbose::VERBOSITY_DEBUG);
}
