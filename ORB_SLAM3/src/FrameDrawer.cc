...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    cv::Scalar keypointColor(0,255,255);  // Yellow color for all keypoints
    ...
    // Draw ALL extracted keypoints in yellow (before drawing tracked ones)
    // if(state==Tracking::OK || state==Tracking::NOT_INITIALIZED || state==Tracking::LOST)
    // {
    //     for(int i=0; i<(int)vCurrentKeys.size(); i++)
    //     {
    //         cv::Point2f pt;
    //         if(imageScale != 1.f)
    //         {
    ...
    //         }
    //         cv::circle(im, pt, 2, keypointColor, -1);  // Small filled yellow circle
    //     }
    // }
