...
public:
    ...
    Frame(const cv::Mat &imGray, const double &timeStamp, const int64_t &timeStampNs, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib &ImuCalib = IMU::Calib());
    ...
    // Add a new keypoint at the specified position and associate it with a MapPoint.
    // Used for optical flow-based tracking where we create keypoints at predicted positions.
    // Computes the ORB descriptor at the keypoint location.
    // Returns the index of the newly added keypoint, or -1 if the keypoint is too close to the image border.
    int AddKeyPoint(int x, int y, int octave, float size, MapPoint* pMP);
...
public:
    ...
    // Grayscale image for dynamic keypoint descriptor computation
    cv::Mat mImGray;

    // umax table for IC_Angle computation (circular patch boundary)
